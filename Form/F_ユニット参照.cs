...
namespace u_net
{
    public partial class F_ユニット参照 : Form
    {
        ...
        public bool BeDetails
        {
            get
            {
                return 0 < ユニット明細参照1.Detail.RowCount;
            }
        }
        ...
        public string CurrentPartsCode
        {
            get
            {
                return ユニット明細参照1.Detail.CurrentRow.Cells["部品コード"].Value?.ToString();
        ...
        public bool GoNewMode()
        {
            try
            {
                ...
                // 明細部の初期化
                strSQL = "SELECT * FROM Vユニット明細参照 WHERE ユニットコード='" +
                             this.CurrentCode + "' AND ユニット版数=" + this.CurrentEdition +
                             " ORDER BY 明細番号";
                VariableSet.SetTable2Details(ユニット明細参照1.Detail, strSQL, cn);
                ...
                // 明細部動作制御
                ユニット明細参照1.Detail.AllowUserToAddRows = true;
                ユニット明細参照1.Detail.AllowUserToDeleteRows = true;
                ユニット明細参照1.Detail.ReadOnly = false;
        ...
        private bool ErrCheck()
        {
            ...
            if ((IsDecided && ユニット明細参照1.Detail.RowCount < 1) || (!IsDecided && ユニット明細参照1.Detail.RowCount <= 1))
            {
            ...
            try
            {
                ...
                switch (controlObject.Name)
                {
                    case "ユニットコード":
                        ...
                        // 明細部の表示
                        strSQL = "SELECT *,IIf(置換不可 = -1,'■','') as 置換不可表示,(単価/入数) as ユニット材料費 FROM Vユニット明細参照 WHERE ユニットコード='" +
                                 this.CurrentCode + "' AND ユニット版数=" + this.CurrentEdition +
                                 " ORDER BY 明細番号";
                        if (!VariableSet.SetTable2Details(ユニット明細参照1.Detail, strSQL, cn))
                        {
                            fn.WaitForm.Close();
                            return;
                        }
                        ...
                        ユニット明細参照1.Detail.AllowUserToAddRows = !this.IsDecided;
                        ユニット明細参照1.Detail.AllowUserToDeleteRows = !this.IsDecided;
                        ユニット明細参照1.Detail.ReadOnly = this.IsDecided;
        ...
        private bool IsBasedRoHS()
        {

            if ((IsDecided && ユニット明細参照1.Detail.RowCount == 0) || (!IsDecided && ユニット明細参照1.Detail.RowCount == 1))
            {
                return false;
            }

            foreach (Row row in ユニット明細参照1.Detail.Rows)
            {
                if (row.IsNewRow)
                {
                    continue;
                ...
                if (string.IsNullOrEmpty(row.Cells["RoHS"].Value?.ToString()))
                {
                    return false;
                }

                if(row.Cells["RoHS"].Value?.ToString() != "〇")
                {
                    return false;
                }
            }
        ...
        private string GetRohsStatus()
        {
            try
            {
                ...
                for (int i = 0; i < ユニット明細参照1.Detail.RowCount; i++)
                {
                    if (ユニット明細参照1.Detail.Rows[i].IsNewRow) continue;

                    if (!Convert.ToBoolean(ユニット明細参照1.Detail.Rows[i].Cells["削除対象"].Value))
                    {
                        int priority = Convert.ToInt32(ユニット明細参照1.Detail.Rows[i].Cells["RohsStatusPriority"].Value);
                        if (priority < stPriority)
                        {
                            stPriority = priority;
                            return ユニット明細参照1.Detail.Rows[i].Cells["RohsStatusSign"].Value.ToString();
                        }
                    }
                }
            }
        ...
        private bool RegTrans(string codeString, int editionNumber, bool updatePreEdition)
        {
            ...
            SqlTransaction transaction = cn.BeginTransaction();
            {
                ...
                try
                {
                    ...
                    // 明細部の登録
                    if (!DataUpdater.UpdateOrInsertDetails(this.ユニット明細参照1.Detail, cn, "Mユニット明細", strwhere, "ユニットコード", transaction))
                    {
                        transaction.Rollback();  // 変更をキャンセル
                        return false;
                    }
                    ...
                    string rohsStatus = IsBasedRoHS() ? "'1'" : "NULL";
        ...
        private bool CopyData(string codeString, int editionNumber)
        {
            try
            {
                ...
                for (int i = 0; i < ユニット明細参照1.Detail.RowCount; i++)
                {
                    if (ユニット明細参照1.Detail.Rows[i].IsNewRow == true)
                    {
                        //新規行の場合は、処理をスキップ
                        continue;
                    }

                    ユニット明細参照1.Detail.Rows[i].Cells["ユニットコード"].Value = codeString;
                    ユニット明細参照1.Detail.Rows[i].Cells["ユニット版数"].Value = editionNumber;
        ...
        private bool ClearHistory()
        {
            try
            {
                ...
                if (ユニット明細参照1.Detail.RowCount == 0)
                {
                    return true;
                }
                ...
                for (int i = 0; i < ユニット明細参照1.Detail.RowCount; i++)
                {
                    ...
                    if (string.IsNullOrEmpty(ユニット明細参照1.Detail.Rows[i].Cells["削除対象"].Value?.ToString()))
                    {
                        ユニット明細参照1.Detail.Rows[i].Cells["削除対象"].Value = false;
                    }

                    if (Convert.ToBoolean(ユニット明細参照1.Detail.Rows[i].Cells["削除対象"].Value))
                    {
                        ユニット明細参照1.Detail.Rows.RemoveAt(i);
                    }
                    else
                    {
                        ユニット明細参照1.Detail.Rows[i].Cells["明細番号"].Value = lngi;
                        ユニット明細参照1.Detail.Rows[i].Cells["変更操作コード"].Value = null;
                        ユニット明細参照1.Detail.Rows[i].Cells["変更内容"].Value = null;
                        lngi++;
                    }
                }
        ...
        private void コマンド確定_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (SaveData())
                {
                    ...
                    ユニット明細参照1.Detail.AllowUserToAddRows = !IsDecided;
                    ユニット明細参照1.Detail.AllowUserToDeleteRows = !IsDecided;
                    ユニット明細参照1.Detail.ReadOnly = IsDecided;
        ...
        private void コマンド部品表_Click(object sender, EventArgs e)
        {
            ...
            while (RowCount > 0)
            {
                ...
                else if (f_ユニット.ユニット明細参照1.Detail.SortOrder != 0)
                {
            ...
            try
            {
                ...
                using (SqlCommand cmdCheck = new SqlCommand(strSQL1, connection))
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        string strSQL3 = $"DELETE FROM Mユニット明細参照 WHERE {strKey}";

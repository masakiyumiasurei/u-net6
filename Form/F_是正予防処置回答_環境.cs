...
namespace u_net
{
    public partial class F_是正予防処置回答_環境 : Form
    {

        public F_是正予防処置回答_環境()
        {
            InitializeComponent();
        }
        ...
        private bool LoadData(Form formObject, string codeString, int editionNumber)
        {
            try
            {
                ...
                strSQL = "SELECT * FROM T是正予防処置回答_環境 WHERE 文書コード ='" + codeString + "'";
        ...
        private bool SaveData(string codeString, int editionNumber)
        {
            ...
            SqlTransaction transaction = cn.BeginTransaction();
            {
                ...
                try
                {
                    ...
                    // ヘッダ部の登録
                    if (!DataUpdater.UpdateOrInsertDataFrom(this, cn, "T是正予防処置回答_環境", strwhere, "文書コード", transaction, "版数"))
                    {
        ...
        private void F_是正予防処置回答_環境_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Return:
        ...
        private void 修正緩和処置_DoubleClick(object sender, EventArgs e)
        {

        }

        private void 修正緩和処置_Enter(object sender, EventArgs e)
        {
            toolStripStatusLabel1.Text = "■全角2,000文字まで入力できます。　■入力欄をダブルクリックすると表示される別ウィンドウ上からも編集できます。";
        }

        private void 修正緩和処置_Leave(object sender, EventArgs e)
        {
            toolStripStatusLabel1.Text = "各種項目の説明";
        }

        private void 修正緩和処置_TextChanged(object sender, EventArgs e)
        {

        }

        private void 修正緩和処置_Validated(object sender, EventArgs e)
        {

        }

        private void 修正緩和処置_Validating(object sender, CancelEventArgs e)
        {

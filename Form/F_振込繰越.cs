...
namespace u_net
{
    public partial class F_振込繰越 : Form
    {
        ...
        private void SetList(DateTime TotalDate, string PayeeCode)
        {
            ...
            string strSQL = "SELECT 明細番号, 繰越金額, 備考  " +
                            "FROM T振込繰越 " +
        ...
        private void 削除ボタン_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string strSQL = "UPDATE T振込繰越 SET 無効日時=GETDATE(),無効者コード='" +
        ...
        private void 追加ボタン_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(繰越金額.Text)) return;
            ...
            try
            {
                ...
                string strSQL = "INSERT T振込繰越 SELECT '" +
                    dtm支払年月.ToString("yyyy/MM/dd") + "','" + str支払先コード + "'," +
                    "ISNULL(MAX(明細番号)+1,1)," +
                    "'" + 備考.Text + "'," +
                    繰越金額.Text + ",NULL,GETDATE(),'" + CommonConstants.LoginUserCode + "',NULL,NULL" +
                    " FROM T振込繰越 WHERE 支払年月='" + dtm支払年月.ToString("yyyy/MM/dd") +
                    "' AND 支払先コード='" + str支払先コード + "'";

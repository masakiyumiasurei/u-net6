...
namespace u_net
{
    public partial class F_製品参照 : Form
    {
        ...
        public bool BeDetails
        {
            get
            {
                return 0 < 製品明細参照1.Detail.RowCount;
            }
        }
        ...
        public bool GoNewMode()
        {
            try
            {
                ...
                // 明細部の初期化
                strSQL = "SELECT * FROM V製品明細参照 WHERE 製品コード='" +
                             this.CurrentCode + "' AND 製品版数=" + this.CurrentEdition +
                             " ORDER BY 明細番号";
                VariableSet.SetTable2Details(製品明細参照1.Detail, strSQL, cn);
        ...
        private bool ErrCheck()
        {
            ...
            if ((IsDecided && 製品明細参照1.Detail.RowCount < 1) || (!IsDecided && 製品明細参照1.Detail.RowCount <= 1))
            {
                ...
                switch (controlObject.Name)
                {
                    case "製品コード":
                        ...
                        strSQL = "SELECT * FROM V製品明細参照 WHERE 製品コード='" +
                                 this.CurrentCode + "' AND 製品版数=" + this.CurrentEdition +
                                 " ORDER BY 明細番号";
                        if (!VariableSet.SetTable2Details(製品明細参照1.Detail, strSQL, cn))
                        {
                            fn.WaitForm.Close();
                            return;
                        }
        ...
        private bool RegTrans(string codeString, int editionNumber, bool updatePreEdition)
        {
            ...
            SqlTransaction transaction = cn.BeginTransaction();
            {
                ...
                try
                {
                    ...
                    // 明細部の登録
                    if (!DataUpdater.UpdateOrInsertDetails(this.製品明細参照1.Detail, cn, "M製品明細参照", strwhere, "製品コード", transaction))
                    {
                        transaction.Rollback();  // 変更をキャンセル
                        return false;
                    }
        ...
        private bool UpdateModelMaster(string productCode, int productEdition, SqlConnection cn, SqlTransaction transaction)
        {
            ...
            try
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    // 型式一覧を取得する
                    //string strSQLSelect = "SELECT 型式名 FROM 製品明細参照 WHERE 削除対象 = FALSE ORDER BY 型式名 DESC";
                    //cmd.CommandText = strSQLSelect;
                    //using (SqlDataReader reader = cmd.ExecuteReader())
                    //{

                    if (製品明細参照1.Detail.RowCount < 1)
                    {
                    ...
                    for (int i = 0; i < 製品明細参照1.Detail.RowCount; i++)
                    {
                        if (製品明細参照1.Detail.Rows[i].IsNewRow == true)
                        {
                            //新規行の場合は、処理をスキップ
                            continue;
                        }

                        if (string.IsNullOrEmpty(製品明細参照1.Detail.Rows[i].Cells["削除対象"].Value?.ToString()))
                        {
                            製品明細参照1.Detail.Rows[i].Cells["削除対象"].Value = false;
                        }

                        if ((bool)製品明細参照1.Detail.Rows[i].Cells["削除対象"].Value)
                        {
                            continue;
                        }

                        string currentModelName = 製品明細参照1.Detail.Rows[i].Cells["型式名"].Value.ToString();
        ...
        private void Form_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            switch (e.KeyCode)
            {
                ...
                case Keys.F6:
                    ...
                    break;
                case Keys.F7:
                    if (コマンド全印刷.Enabled) コマンド全印刷_Click(sender, e);
                    break;
                case Keys.F8:
        ...
        private bool CopyData(string codeString, int editionNumber)
        {
            try
            {
                ...
                for (int i = 0; i < 製品明細参照1.Detail.RowCount; i++)
                {
                    ...
                    製品明細参照1.Detail.Rows[i].Cells["製品コード"].Value = codeString;
                    製品明細参照1.Detail.Rows[i].Cells["製品版数"].Value = editionNumber;
        ...
        private bool ClearHistory()
        {
            try
            {
                ...
                for (int i = 0; i < 製品明細参照1.Detail.RowCount; i++)
                {
                    ...
                    if (Convert.ToBoolean(製品明細参照1.Detail.Rows[i].Cells["削除対象"].Value))
                    {
                        製品明細参照1.Detail.Rows.RemoveAt(i);
                    }
                    else
                    {
                        製品明細参照1.Detail.Rows[i].Cells["明細番号"].Value = lngi;
                        製品明細参照1.Detail.Rows[i].Cells["変更操作コード"].Value = null;
                        製品明細参照1.Detail.Rows[i].Cells["変更内容"].Value = null;
                        lngi++;
                    }
                }
            ...
            try
            {
                ...
                using (SqlCommand cmdCheck = new SqlCommand(strSQL1, connection))
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        string strSQL3 = $"DELETE FROM M製品明細参照 WHERE {strKey}";
        ...
        private bool UndoRevise(SqlConnection connection, string codeString, int editionNumber)
        {
            ...
            try
            {
                ...
                string strSQL1 = $"DELETE FROM M製品明細参照 WHERE {strKey1}";
        ...
        private void コマンド全印刷_Click(object sender, EventArgs e)
        {
            ...
            foreach (Row row in f_製品.製品明細参照1.Detail.Rows)
            {
                ...
                while (RowCount > 0)
                {
                    ...
                    else if (f_製品.製品明細参照1.Detail.SortOrder != 0)
                    {
        ...
        private bool IsBasedRoHS()
        {
            ...
            if ((IsDecided && 製品明細参照1.Detail.RowCount == 0) || (!IsDecided && 製品明細参照1.Detail.RowCount == 1))
            {
                return false;
            }

            foreach (Row row in 製品明細参照1.Detail.Rows)
            {
                if (row.IsNewRow)
                {
                    continue;
                ...
                if (string.IsNullOrEmpty(row.Cells["RoHS対応"].Value?.ToString()))
                {
                    return false;
                }
            }
